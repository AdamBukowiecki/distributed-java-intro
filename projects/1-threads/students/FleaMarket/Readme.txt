...
People called *donors* can bring their old junk to the market and it will be given away on *auctions*. 
Auctions are scheduled and led by a *chairman*. People who take part in auctions are called *recipients*. 
Only limited number of recipients can register for particular auction. 
Registration time for each auction is fixed. 
Once registration is closed, chairman draws a lucky recipient who gets the item. 
After that recipients can register for another auction and the cycle continues. 
Chairman can decide to finish all auctions if there are no items from donors.
